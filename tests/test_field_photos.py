...
class FieldPhotoTests(unittest.TestCase):
    ...
    def test_save_field_photo_uses_map_point_without_exif_gps(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIsNone(record.get("vehicle_resolution_updated_at"))
            self.assertEqual(photo["vehicle_insurance_status"], "unknown")
            self.assertIsNone(photo.get("vehicle_insurance_checked_at"))
            self.assertEqual(photo["vehicle_resolution_status"], "active")
            self.assertIsNone(photo.get("vehicle_resolution_updated_at"))
            ...
            self.assertEqual(pending_list[0]["vehicle_resolution_status"], "active")
