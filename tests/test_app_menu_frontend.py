...
class AppMenuFrontendContracts(unittest.TestCase):
    def test_app_menu_uses_compact_footer_actions(self):
        ...
        self.assertIn("cycleOverlayFocus(event, [toggle, ...visibleFocusableElements(drawer)]", ui_js)
        self.assertIn('id="app-menu-scrim" type="button" tabindex="-1" aria-hidden="true"', html)
        self.assertIn('id="status" class="global-status-toast" role="status"', html)
