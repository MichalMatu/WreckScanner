...
from core.report_assets import report_package_asset
...
class ReadForbidden(BytesIO):
    def read(self, *args, **kwargs):
        raise AssertionError("Request body should not be read for this test.")


def admin_cookie():
    token = auth.make_admin_token("secret")
...
class JsonResponseRequestIdContractTests(unittest.TestCase):
    def test_error_json_includes_request_id_and_response_header(self):
        ...
        responses.send_json(handler, 400, {"error": "bad payload"})
    ...
    def test_success_json_keeps_payload_shape_and_sanitizes_request_id_header(self):
        ...
        responses.send_json(handler, 200, {"status": "ok"})
...
class ApiNotFoundContractTests(unittest.TestCase):
    ...
    def test_unknown_head_api_endpoint_returns_headers_without_body(self):
        ...
        http_server.Handler.do_HEAD(handler)
    ...
    def test_unknown_post_api_endpoint_returns_json_not_found(self):
        ...
        http_server.Handler.do_POST(handler)
    ...
    def test_unknown_patch_api_endpoint_returns_json_not_found(self):
        ...
        http_server.Handler.do_PATCH(handler)
    ...
    def test_unknown_delete_api_endpoint_returns_json_not_found(self):
        ...
        http_server.Handler.do_DELETE(handler)

        self._assert_api_not_found(handler, "missing-delete-req")


class JsonBodyLimitContractTests(unittest.TestCase):
    def test_oversized_json_post_returns_413_without_reading_body(self):
        handler = make_handler(
            "/api/settings",
            headers={
                **admin_cookie(),
                "Content-Length": str(app_config.MAX_JSON_BODY_BYTES + 1),
                "X-Request-ID": "json-too-large-post",
            },
            method="POST",
        )
        ...
        self.assertEqual(payload["request_id"], "json-too-large-post")
        self.assertEqual(payload["limit_bytes"], app_config.MAX_JSON_BODY_BYTES)
        self.assertIn("Payload JSON przekracza limit", payload["error"])

    def test_oversized_json_patch_returns_413_without_reading_body(self):
        handler = make_handler(
            "/api/admin/privacy-requests/privacy_20260611_deadbeef",
            headers={
                **admin_cookie(),
                "Content-Length": str(app_config.MAX_JSON_BODY_BYTES + 1),
                "X-Request-ID": "json-too-large-patch",
            },
            method="PATCH",
        )
        handler.rfile = ReadForbidden()
        ...
        payload = handler_json(handler)
        self.assertEqual(handler.status, 413)
        self.assertEqual(payload["request_id"], "json-too-large-patch")
        self.assertEqual(payload["limit_bytes"], app_config.MAX_JSON_BODY_BYTES)

    def test_invalid_json_content_length_returns_400_without_reading_body(self):
        handler = make_handler(
            "/api/download",
            headers={"Content-Length": "not-a-number", "X-Request-ID": "bad-content-length"},
            method="POST",
        )
        handler.rfile = ReadForbidden()
        ...
        self.assertEqual(payload["request_id"], "bad-content-length")
...
class SettingsApiContractTests(unittest.TestCase):
    ...
    def test_post_settings_unexpected_error_hides_details(self):
        ...
        with (
            patch.dict(os.environ, {"WRECKSCANNER_ADMIN_PASSWORD": "secret"}),
            patch.object(http_settings, "save_app_settings", side_effect=OSError("settings token=secret")),
...
class CadastralApiContractTests(unittest.TestCase):
    def test_cadastral_identify_uses_fixed_upstream_and_returns_parcel_json(self):
        ...
        with (
            patch.object(http_access, "load_app_settings", return_value=settings_store.default_app_settings()),
            patch.object(map_downloads, "get_http_session", return_value=fake_session),
        ):
            http_server.Handler.do_GET(handler)
    ...
    def test_cadastral_identify_rejects_invalid_coordinates(self):
        ...
        with patch.object(http_access, "load_app_settings", return_value=settings_store.default_app_settings()):
            http_server.Handler.do_GET(handler)
    ...
    def test_cadastral_identify_obeys_public_layer_settings(self):
        handler = make_handler("/api/cadastral/identify?lat=51.089742&lon=17.038940")

        with (
            patch.object(http_access, "load_app_settings", return_value={"public_layers": {"cadastral": False}}),
            patch.object(map_downloads, "get_http_session") as session_factory,
        ):
            http_server.Handler.do_GET(handler)

        self.assertEqual(handler.status, 403)
        self.assertIn("dzialek", handler_json(handler)["error"])
        session_factory.assert_not_called()


class SurfaceApiContractTests(unittest.TestCase):
    def test_surface_features_obeys_public_layer_settings(self):
        handler = make_handler("/api/surface/features?bbox=51.0,17.0,51.1,17.1")

        with (
            patch.object(http_access, "load_app_settings", return_value={"public_layers": {"surface": False}}),
            patch.object(http_public_data, "surface_features_geojson") as surface_lookup,
        ):
            http_server.Handler.do_GET(handler)

        self.assertEqual(handler.status, 403)
        self.assertIn("nawierzchni", handler_json(handler)["error"])
        surface_lookup.assert_not_called()
    ...
    def test_surface_features_hides_internal_exception_details_and_returns_empty_geojson(self):
        ...
        with (
            ...
            patch.object(
                http_public_data,
                "surface_features_geojson",
                side_effect=RuntimeError("overpass token=secret failed"),
            ),
            self.assertLogs("wreckscanner.server", level="ERROR") as logs,
        ):
            http_server.Handler.do_GET(handler)
...
class StatusApiContractTests(unittest.TestCase):
    def test_public_admin_status_does_not_expose_pending_submission_usage(self):
        ...
        with (
            patch.dict(os.environ, {"WRECKSCANNER_ADMIN_PASSWORD": "secret"}),
            patch.object(http_admin, "pending_submission_usage", side_effect=AssertionError("should not scan storage")),
        ):
            http_server.Handler.do_GET(handler)
    ...
    def test_authenticated_admin_status_includes_pending_submission_usage(self):
        ...
        with (
            patch.dict(os.environ, {"WRECKSCANNER_ADMIN_PASSWORD": "secret"}),
            patch.object(http_admin, "pending_submission_usage", return_value=usage) as usage_mock,
        ):
            http_server.Handler.do_GET(handler)
    ...
    def test_admin_status_unexpected_error_hides_details(self):
        ...
        with (
            patch.dict(os.environ, {"WRECKSCANNER_ADMIN_PASSWORD": "secret"}),
            patch.object(http_admin, "pending_submission_usage", side_effect=OSError("admin status token=secret")),
            self.assertLogs("wreckscanner.server", level="ERROR") as logs,
        ):
            http_server.Handler.do_GET(handler)
...
class SecurityHeadersContractTests(unittest.TestCase):
    def test_default_security_headers_are_conservative_and_global(self):
        headers = responses.security_response_headers()
    ...
    def test_admin_cookie_is_secure_on_public_hosts_but_not_localhost(self):
        ...
        with patch.dict(os.environ, {"WRECKSCANNER_ADMIN_PASSWORD": "secret"}):
            http_server.Handler.do_POST(public_handler)
            http_server.Handler.do_POST(local_handler)
    ...
    def test_cors_headers_are_origin_whitelisted_without_wildcard(self):
        self.assertEqual(responses.cors_response_headers("https://evil.example"), {})
        allowed = responses.cors_response_headers("https://wreckscanner.pl")
    ...
    def test_options_response_has_request_id_and_empty_body(self):
        ...
        http_server.Handler.do_OPTIONS(handler)
...
class ListApiFailureContractTests(unittest.TestCase):
    def test_admin_photo_queue_unexpected_error_hides_details(self):
        ...
        with (
            patch.dict(os.environ, {"WRECKSCANNER_ADMIN_PASSWORD": "secret"}),
            patch.object(http_admin, "list_field_photo_review_items", side_effect=OSError("photo queue token=secret")),
            self.assertLogs("wreckscanner.server", level="ERROR") as logs,
        ):
            http_server.Handler.do_GET(handler)
    ...
    def test_admin_wreck_queue_unexpected_error_hides_details(self):
        ...
        with (
            patch.dict(os.environ, {"WRECKSCANNER_ADMIN_PASSWORD": "secret"}),
            patch.object(http_admin, "list_wreck_review_items", side_effect=OSError("wreck queue token=secret")),
            self.assertLogs("wreckscanner.server", level="ERROR") as logs,
        ):
            http_server.Handler.do_GET(handler)
    ...
    def test_admin_privacy_queue_unexpected_error_hides_details(self):
        ...
        with (
            patch.dict(os.environ, {"WRECKSCANNER_ADMIN_PASSWORD": "secret"}),
            patch.object(http_admin, "list_privacy_requests", side_effect=OSError("privacy queue token=secret")),
            self.assertLogs("wreckscanner.server", level="ERROR") as logs,
        ):
            http_server.Handler.do_GET(handler)
    ...
    def test_wreck_list_unexpected_error_hides_details(self):
        ...
        with (
            patch.object(http_public_data, "list_wrecks", side_effect=OSError("wreck list token=secret")),
            self.assertLogs("wreckscanner.server", level="ERROR") as logs,
        ):
            http_server.Handler.do_GET(handler)
    ...
    def test_field_photo_list_unexpected_error_hides_details(self):
        ...
        with (
            patch.object(http_public_data, "list_field_photos", side_effect=OSError("field list token=secret")),
            self.assertLogs("wreckscanner.server", level="ERROR") as logs,
        ):
            http_server.Handler.do_GET(handler)
...
class DownloadApiContractTests(unittest.TestCase):
    ...
    def test_versioned_route_url_appends_cache_buster(self):
        self.assertEqual(
            http_scan.versioned_route_url("/analiza/report.html", "123"),
            "/analiza/report.html?v=123",
        )
        self.assertEqual(
            http_scan.versioned_route_url("/analiza/report.html?lang=pl", "123"),
    ...
    def test_download_progress_callback_updates_percent_and_completion(self):
        ...
        with (
            ...
            patch.object(pipeline, "system_pressure", return_value={"overloaded": False}),
            patch.object(
                http_scan, "set_download_progress", side_effect=lambda **payload: progress_events.append(payload)
            ),
            patch("builtins.print") as print_mock,
        ):
            http_server.Handler.do_POST(handler)
    ...
    def test_download_unexpected_error_hides_details_and_releases_slot(self):
        ...
        payload = handler_json(handler)
        progress = http_scan.get_download_progress()
        self.assertEqual(handler.status, 500)
    ...
    def test_inspect_unexpected_error_hides_details(self):
        ...
        with (
            patch.object(http_scan, "save_scan_crops", side_effect=OSError("inspect token=secret")),
            self.assertLogs("wreckscanner.server", level="ERROR") as logs,
        ):
            http_server.Handler.do_POST(handler)
    ...
    def test_analyze_success_json_has_request_headers_without_payload_change(self):
        ...
        with TemporaryDirectory() as tmp:
            ...
            completed = http_scan.subprocess.CompletedProcess(
                ["python", "analyze.py"],
                0,
                stdout="analysis ok",
                stderr="",
            ...
            with (
                ...
                patch.object(pipeline, "system_pressure", return_value={"overloaded": False}),
                patch.object(http_scan.subprocess, "run", return_value=completed),
                patch("builtins.print") as print_mock,
            ):
                http_server.Handler.do_POST(handler)
...
class PrivacyRequestApiContractTests(unittest.TestCase):
    def test_privacy_pages_and_report_request_queue(self):
        with TemporaryDirectory() as tmp:
            ...
            http_server.Handler.do_GET(privacy_page)
            http_server.Handler.do_HEAD(privacy_head)
            http_server.Handler.do_GET(report_page)
            http_server.Handler.do_HEAD(report_head)
            ...
            with (
                ...
                http_server.Handler.do_POST(create_handler)
                http_server.Handler.do_GET(public_queue)
                http_server.Handler.do_GET(admin_queue)
                ...
                http_server.Handler.do_PATCH(update_handler)
                filtered_queue = make_handler(
                    "/api/admin/privacy-requests?status=in_progress",
                    headers=admin_cookie(),
                )
                http_server.Handler.do_GET(filtered_queue)
    ...
    def test_privacy_request_update_unexpected_error_hides_details(self):
        ...
        with (
            patch.dict(os.environ, {"WRECKSCANNER_ADMIN_PASSWORD": "secret"}),
            patch.object(http_admin, "update_privacy_request", side_effect=OSError("privacy token=secret")),
            self.assertLogs("wreckscanner.server", level="ERROR") as logs,
        ):
            http_server.Handler.do_PATCH(handler)
    ...
    def test_privacy_request_create_unexpected_error_hides_details(self):
        ...
        with (
            patch.object(http_public, "create_privacy_request", side_effect=OSError("privacy create token=secret")),
            self.assertLogs("wreckscanner.server", level="ERROR") as logs,
        ):
            http_server.Handler.do_POST(handler)
...
class PhotoRetentionApiContractTests(unittest.TestCase):
    def _reset_retention_state(self) -> None:
        with http_retention._photo_retention_state_lock:
            http_retention._photo_retention_state.update(
                {
                    "running": False,
    ...
    def test_admin_can_view_and_run_photo_retention(self):
        ...
        with (
            patch.dict(os.environ, {"WRECKSCANNER_ADMIN_PASSWORD": "secret"}),
            patch.object(http_retention, "retire_private_originals", return_value=fake_report) as retire_mock,
        ):
            http_server.Handler.do_GET(public_status)
            http_server.Handler.do_GET(admin_status)
            http_server.Handler.do_POST(run_handler)
    ...
    def test_photo_retention_unexpected_error_hides_details_from_response_snapshot(self):
        ...
        with (
            patch.dict(os.environ, {"WRECKSCANNER_ADMIN_PASSWORD": "secret"}),
            patch.object(http_retention, "retire_private_originals", side_effect=OSError("retention token=secret")),
            self.assertLogs("wreckscanner.server", level="ERROR") as logs,
        ):
            http_server.Handler.do_POST(handler)
...
class ReportPackageApiContractTests(unittest.TestCase):
    ...
    def test_report_package_requires_admin_before_form_validation(self):
        ...
        with patch.dict(os.environ, {"WRECKSCANNER_ADMIN_PASSWORD": "secret"}):
            http_server.Handler.do_POST(public_handler)
            http_server.Handler.do_POST(admin_handler)
    ...
    def test_public_report_package_generates_clean_tokenized_download(self):
        with TemporaryDirectory() as tmp:
            ...
            with patch.object(core_config, "PRIVATE_REPORTS_DIR", private_reports_dir):
                http_server.Handler.do_GET(public_zip)
                http_server.Handler.do_GET(bad_zip)
    ...
    def test_public_report_package_unexpected_error_hides_details(self):
        ...
        with (
            patch.object(http_request_body, "read_multipart_form", return_value=({"sender": "Jan"}, [])),
            patch.object(
                http_public,
        ...
        with TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch.object(
                    http_access,
                    "load_app_settings",
                    return_value=app_settings_with_disabled_feature("photo_uploads"),
                ),
            ):
                http_server.Handler.do_POST(handler)
    ...
    def test_save_wreck_endpoint_is_public(self):
        ...
        with (
            ...
            patch.object(http_access, "assert_pending_submission_quota"),
        ):
            http_server.Handler.do_POST(handler)
    ...
    def test_save_wreck_unexpected_error_hides_details(self):
        ...
        with (
            patch.object(http_access, "assert_pending_submission_quota"),
            patch.object(http_public, "save_manual_wreck", side_effect=OSError("save wreck token=secret")),
            self.assertLogs("wreckscanner.server", level="ERROR") as logs,
        ):
            http_server.Handler.do_POST(handler)
    ...
    def test_public_feature_settings_block_public_scan_and_manual_wrecks(self):
        ...
        with patch.object(
            http_access,
            "load_app_settings",
            return_value=app_settings_with_disabled_feature("scan_analysis"),
        ):
            http_server.Handler.do_POST(download)
            http_server.Handler.do_POST(analyze)
        with patch.object(
            http_access, "load_app_settings", return_value=app_settings_with_disabled_feature("yolo_wrecks")
        ):
            http_server.Handler.do_POST(yolo_wreck)
        with patch.object(
            http_access,
            "load_app_settings",
            return_value=app_settings_with_disabled_feature("manual_wrecks"),
        ):
            http_server.Handler.do_POST(manual)
    ...
    def test_wreck_review_unexpected_error_hides_details(self):
        ...
        with (
            patch.dict(os.environ, {"WRECKSCANNER_ADMIN_PASSWORD": "secret"}),
            patch.object(http_admin, "review_wreck", side_effect=OSError("wreck review token=secret")),
            self.assertLogs("wreckscanner.server", level="ERROR") as logs,
        ):
            http_server.Handler.do_PATCH(handler)
    ...
    def test_wreck_layer_visibility_filters_guest_api_only(self):
        ...
        with (
            patch.dict(os.environ, {"WRECKSCANNER_ADMIN_PASSWORD": "secret"}),
            patch.object(http_access, "load_app_settings", return_value=settings),
            patch.object(http_public_data, "list_wrecks", return_value=wrecks),
        ):
            http_server.Handler.do_GET(guest)
            http_server.Handler.do_GET(admin)
    ...
    def test_report_package_endpoint_accepts_multipart_and_generates_zip(self):
        with TemporaryDirectory() as tmp:
            ...
            with (
                ...
                http_server.Handler.do_GET(public_zip_get)
                http_server.Handler.do_GET(zip_get)
                http_server.Handler.do_GET(pdf_get)
    ...
    def test_wreck_photo_upload_allows_public_pending_and_admin_review(self):
        with TemporaryDirectory() as tmp:
            ...
            with (
                ...
                http_server.Handler.do_POST(public_upload)
            ...
            with patch.object(core_config, "WRECKS_DIR", wrecks_dir):
                http_server.Handler.do_GET(public_original)
                http_server.Handler.do_HEAD(public_original_head)
                http_server.Handler.do_GET(record_json)
                http_server.Handler.do_HEAD(record_json_head)
            ...
            with (
                ...
                http_server.Handler.do_GET(no_admin_original)
                http_server.Handler.do_GET(admin_original)
                http_server.Handler.do_PATCH(approve)
            ...
            with (
                ...
                http_server.Handler.do_GET(public_thumb)
                http_server.Handler.do_HEAD(public_thumb_head)
            ...
            with (
                ...
                http_server.Handler.do_PATCH(reject)
                http_server.Handler.do_DELETE(no_admin_delete)
                http_server.Handler.do_DELETE(delete_photo)
    ...
    def test_wreck_index_get_refreshes_public_report_html(self):
        with TemporaryDirectory() as tmp:
            ...
            with patch.object(core_config, "WRECKS_DIR", wrecks_dir):
                http_server.Handler.do_GET(handler)
                http_server.Handler.do_HEAD(head_handler)
    ...
    def test_public_wreck_asset_unexpected_error_hides_details(self):
        ...
        with (
            patch.object(http_assets, "wreck_is_public", return_value=True),
            patch.object(http_assets, "public_wreck_asset", side_effect=OSError("wreck asset token=secret")),
            self.assertLogs("wreckscanner.server", level="ERROR") as logs,
        ):
            http_server.Handler.do_GET(handler)
...
class FieldPhotoApiContractTests(unittest.TestCase):
    ...
    def test_field_photo_upload_unexpected_error_hides_details(self):
        ...
        with (
            patch.object(
                ...
                "read_multipart_form",
                return_value=(
                    {"fallback_lat": "51.1", "fallback_lon": "17.1", "edit_token": "owner-token-123"},
                    [uploaded],
                ),
            ),
            patch.object(http_access, "ensure_public_submission_quota", return_value=None),
            patch.object(http_public, "save_field_photo", side_effect=RuntimeError("field photo token=secret")),
            self.assertLogs("wreckscanner.server", level="ERROR") as logs,
        ):
            http_server.Handler.do_POST(handler)
    ...
    def test_field_photo_asset_unexpected_error_hides_details(self):
        ...
        with (
            patch.object(http_assets, "field_photo_asset", side_effect=OSError("field asset token=secret")),
            self.assertLogs("wreckscanner.server", level="ERROR") as logs,
        ):
            http_server.Handler.do_GET(handler)
    ...
    def test_field_photo_delete_unexpected_error_hides_details(self):
        ...
        with (
            patch.dict(os.environ, {"WRECKSCANNER_ADMIN_PASSWORD": "secret"}),
            patch.object(http_admin, "delete_field_photo", side_effect=OSError("field delete token=secret")),
            self.assertLogs("wreckscanner.server", level="ERROR") as logs,
        ):
            http_server.Handler.do_DELETE(handler)
    ...
    def test_field_photo_upload_allows_public_pending_submission(self):
        with TemporaryDirectory() as tmp:
            ...
            edit_token = "owner-token-123"
            handler = make_multipart_handler(
                "/api/field-photos",
                fields={"fallback_lat": "51.1", "fallback_lon": "17.2", "edit_token": edit_token},
            ...
            with (
                ...
                http_server.Handler.do_GET(list_handler)
            ...
            self.assertNotIn(edit_token, json.dumps(record))
    ...
    def test_public_field_photo_upload_requires_edit_token(self):
        handler = make_multipart_handler(
            "/api/field-photos",
        ...
        self.assertEqual(handler.status, 400)
        self.assertIn("token", handler_json(handler)["error"].lower())

    def test_field_photo_owner_token_unlocks_original_and_redaction_save(self):
        with TemporaryDirectory() as tmp:
            field_photos_dir = Path(tmp) / "zdjecia_terenowe"
            private_photos_dir = Path(tmp) / "private_photos"
            edit_token = "owner-token-123"
            ...
            with (
                ...
                http_server.Handler.do_POST(upload_handler)
                photo_id = handler_json(upload_handler)["photo"]["id"]

                bad_claim = make_handler(
                    "/api/field-photos/owner-claim",
                    {"photo_ids": [photo_id], "edit_token": "wrong-token"},
                )
                http_server.Handler.do_POST(bad_claim)

                claim_handler = make_handler(
                    "/api/field-photos/owner-claim",
                    {"photo_ids": [photo_id], "edit_token": edit_token},
                )
                http_server.Handler.do_POST(claim_handler)

                original_handler = make_handler(
                    f"/api/field-photos/{photo_id}/owner-original",
                    {"edit_token": edit_token},
                )
                http_server.Handler.do_POST(original_handler)

                owner_review = make_handler(
                    f"/api/field-photos/{photo_id}/owner-review",
                    {
                        "edit_token": edit_token,
                ...
                http_server.Handler.do_PATCH(owner_review)
                record = json.loads((field_photos_dir / photo_id / "record.json").read_text(encoding="utf-8"))

            self.assertEqual(bad_claim.status, 403)
            self.assertEqual(claim_handler.status, 200)
            self.assertEqual(handler_json(claim_handler)["photos"][0]["photo_id"], photo_id)
            self.assertEqual(original_handler.status, 200)
            self.assertIn(("Content-Type", "image/jpeg"), original_handler.response_headers)
            self.assertEqual(original_handler.wfile.getvalue(), image_bytes())
            self.assertEqual(owner_review.status, 200)
            self.assertEqual(handler_json(owner_review)["photo"]["public_review_status"], "pending")
            self.assertEqual(record["public_review_status"], "pending")
    ...
    def test_public_layer_settings_block_public_field_photo_issue_type_uploads(self):
        handler = make_multipart_handler(
            "/api/field-photos",
            fields={
                "fallback_lat": "51.1",
        ...
        with patch.object(
            http_access,
            "load_app_settings",
            return_value={"public_layers": {"field_photo_smoke": False}},
        ):
            http_server.Handler.do_POST(handler)

        self.assertEqual(handler.status, 403)
        self.assertIn("kategoria", handler_json(handler)["error"])
    ...
    def test_field_photo_layer_visibility_filters_guest_api_only(self):
        ...
        with (
            patch.dict(os.environ, {"WRECKSCANNER_ADMIN_PASSWORD": "secret"}),
            patch.object(http_access, "load_app_settings", return_value=settings),
            patch.object(http_public_data, "list_field_photos", return_value=photos),
        ):
            http_server.Handler.do_GET(guest)
            http_server.Handler.do_GET(admin)
    ...
    def test_admin_photo_queue_filters_status_scope_issue_and_search(self):
        ...
        with (
            patch.dict(os.environ, {"WRECKSCANNER_ADMIN_PASSWORD": "secret"}),
            patch.object(http_admin, "list_field_photo_review_items", return_value=field_photos),
            patch.object(http_admin, "list_wreck_photo_review_items", return_value=wreck_photos),
        ):
            http_server.Handler.do_GET(handler)
    ...
    def test_field_photo_list_assets_are_public_and_delete_requires_admin(self):
        with TemporaryDirectory() as tmp:
            ...
            with (
                ...
                http_server.Handler.do_GET(review_exact)
                http_server.Handler.do_GET(review_missing)
                ...
                http_server.Handler.do_GET(public_image)
                ...
                http_server.Handler.do_DELETE(delete_handler)
    ...
    def test_field_photo_location_patch_requires_admin_and_updates_record(self):
        with TemporaryDirectory() as tmp:
            ...
            with (
                ...
                http_server.Handler.do_PATCH(public_patch)
                ...
                http_server.Handler.do_PATCH(invalid_patch)
                ...
                http_server.Handler.do_GET(reviewed_list)
    ...
    def test_field_photos_can_be_moved_to_wreck_folder_and_removed_from_field_layer(self):
        with TemporaryDirectory() as tmp:
            ...
            with (
                ...
                http_server.Handler.do_POST(public_attach)
                ...
                http_server.Handler.do_POST(attach_handler)
